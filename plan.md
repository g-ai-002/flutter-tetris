...
- **目标**: 多种游戏模式（限时、挑战）
- **任务**:
  - [x] 实现 GameMode 枚举与模式配置（经典/限时/挑战）
  - [x] 扩展 GameState 支持限时倒计时和挑战目标计数
  - [x] 扩展 GameProvider 支持模式切换和模式特有逻辑
  - [x] 实现模式选择页面 ModeSelectPage
  - [x] 更新 GamePage 集成模式选择和模式特有 UI
  - [x] 补充测试用例
  - [x] 代码审查与清理
  - [x] 更新版本号至 0.5.0
  - [x] 更新 README
...
- **任务**:
  - [x] 提取 PortraitLayout/LandscapeLayout 公共布局组件（InfoPanel/ControlArea）
  - [x] 消除 SoundService WAV 生成重复代码（_tone/_sweep 合并为 _generateWav）
  - [x] 消除 GameProvider moveLeft/moveRight 重复（合并为 _moveHorizontal）
  - [x] 提取测试公共工具文件 test_utils.dart
  - [x] BoardPainter 提取通用 _drawShape 方法
  - [x] 拆分 app_theme.dart _buildTheme 过长方法
  - [x] 提取日期格式化工具类 DateFormatUtils
  - [x] 修复 _clearLines 使用 b.length 替代 height 硬编码
  - [x] 补充测试用例
  - [x] 代码审查与清理
  - [x] 更新版本号至 0.4.2
  - [x] 更新 README
...
- **目标**: 排行榜/历史记录
- **任务**:
  - [x] 实现 HistoryService：存储/读取游戏历史记录（JSON 文件持久化）
  - [x] 实现 HistoryPage：历史记录/排行榜页面（按分数降序排列）
  - [x] 集成到 GamePage：游戏结束时自动保存记录，AppBar 添加历史记录入口
  - [x] 支持清空历史记录
  - [x] 补充测试用例
  - [x] 代码审查与清理
  - [x] 更新版本号至 0.4.0
